...
local function evaluate(rowName, newTicks)
    ...
    local tickCount, badCount = 0, badTicks[rowName]
    ...
    while tick.next do
        ...
        if minSpread < 0 then
            badCount = (badCount or 0) + 1
            badTicks[rowName] = badCount
            gs.logMessage(gs.logLevel.INFO, 
                "bad tick at ", gh.formatTime(nil, tick.timeFirst), 
                "; got ", badCount, " so far for ", rowName)
        end
    end
    ...
    return { numTicks = tickCount, lastTrade = lastTrade,
        ...
        maxInterval = round2dp(maxInterval), badTicks = badCount }
end
